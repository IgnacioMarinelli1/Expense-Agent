...
<div class="mx-auto flex h-full w-full flex-col">
    <div class="flex flex-1 flex-col gap-3 overflow-y-auto p-4">
        {#each $messages as message (message.id)}
            <div
                class="flex items-end gap-2 {message.type === 'usuario'
                    ? 'justify-end'
                    : 'justify-start'}"
            >
                {#if message.type === "agente"}
                    <div
                ...
                {/if}
                <div
                    class="max-w-[80%] px-4 py-2.5 text-sm leading-relaxed shadow-sm {message.type ===
                    'usuario'
                ...
                >
                    ...
                    {:else if message.fileUrl}
                        <div class="flex flex-col gap-2">
                            {#if message.fileType === "pdf"}
                                <div
                                    class="flex items-center gap-2 pb-1 border-b border-primary/20"
                                >
                                ...
                                <embed
                                    ...
                                    class="h-[300px] w-full min-w-[200px] sm:h-[400px] sm:w-[350px] rounded-md bg-white object-cover shadow-inner"
                                />
                            {:else}
                                <img
                                    src={message.fileUrl}
                                    alt="Preview"
                                    class="max-h-[300px] max-w-full sm:max-h-[400px] sm:max-w-[350px] rounded-md object-contain shadow-inner"
                                />
                            {/if}
                            {#if message.text}
                    ...
                    {:else}
                        <div
                            class="markdown-body text-sm {message.loading
                                ? 'streaming-cursor'
                                : ''}"
                        >
                            {@html marked.parse(message.text)}
